					...
					e2 =>
						{
							...
							try
							{
								group = url.toLowerCase();
								if (!group) { debugger; }
								group = group.match(/[a-zа-я0-9_\.]*$/)[0].replace(/\.html?/,'').replace(/_/g,'.');
							}
							catch (e)
							{
							...
							try
							{
								switch (e2.length)
								{
									case 7:
										...
										{	// two swithing with one lecturer
											...
											{	// person in the first part
												var href = eNew[2].parent.attribs.href;
												if (!href) { debugger; }
												personId = +href.match(/[0-9].*.htm/)[0].replace('.htm', '');
												person = eNew[2].data;

												return [
													{ time, name: eNew[0].data, place: clearPlace(eNew[1].data), person, personId, group, position: 1, status: 1 },
													{ time, name: eNew[3].data, place: clearPlace(eNew[4].data), person, personId, group, position: 2,  status: 1 }
												];
											}
											else if (eNew[4].parent.name === 'a')
											{
												var href = eNew[4].parent.attribs.href;
												if (!href) { debugger; }
												personId = +href.match(/[0-9].*.htm/)[0].replace('.htm', '');
												person = eNew[4].data;
												...
												return [
													{ time, name: eNew[0].data, place: clearPlace(eNew[1].data), person, personId, group, position: 1,  status: 1 },
													{ time, name: eNew[2].data, place: clearPlace(eNew[3].data), person, personId, group, position: 2,  status: 1 }
												];
											}
											else { debugger; }
										}
										...
										return { time, name, place, person, personId, group, position, status: 1 }

									case 3:				// all data were provided
										var href = e2[2].parent.attribs.href;
										if (!href) { debugger; }
										return {
											time,
											name: e2[0].data,
									...
									case 2:				// missing one param, guess it's a person, but can be wrong
										...
										{
											var href = e2[1].parent.attribs.href;
											if (!href) { debugger; }
										...
										return {
											name: e2[0].data,
											time, place, person, personId, group, position,
											status: 1
										}
									case 1:				// shoud be empty, but in case it's a class without a tutor and a place
										...
										else
										{
											return {
												name: e2[0].data,
												time, place: '', person: '', personId: 0, group, position,
												status: 1
											}
										}
