...
function handleTeacherTable($, url)
{
  return new Promise(resolve =>
  {
    ...
    try
    {
      ...
      let name = namesMap[shortName.replace(/\./g, '')];
    ...
    .then(name =>
    {
      ...
      if (arr.length > 0)
      {
        let initials = shortName.match(/[А-Я]/g);
        name = shortName;
      ...
      namesMap[shortName.replace(/\./g, '')] = name;
...
function createNameLists()
{
  ...
  for (let key of Object.keys(groupsMap))
  {
    ...
    newNameLists.groups.push(item.groupId);
    newNameLists.tsp = Math.max(newNameLists.tsp, item.tsp);
  }
  for (let key of Object.keys(teachersMap))
  {
    let item = teachersMap[key];
    newNameLists.teachers.push({name: item.name, teacherId: item.teacherId});
    newNameLists.tsp = Math.max(newNameLists.tsp, item.tsp);
  }
  for (let key of Object.keys(placesMap))
  {
    let item = placesMap[key];
    newNameLists.places.push(item.placeId);
    newNameLists.tsp = Math.max(newNameLists.tsp, item.tsp);
  }
