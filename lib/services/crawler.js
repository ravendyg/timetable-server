  ...
  for (let i = 0; i < links.length; i++)
  {
    ...
    if (/[^\/]*\.html?$/.test(_url))
    {
      try
      {
        let link = url + _url;
        let time = Date.parse(links[i].parent.next.children[0].data);
        let _time =   groupsListMap.get(link);
        if (!_time || _time < time)
        { // expired or new
          out.push(link);
            groupsListMap.set(link, time);
        }
      }
      catch (err)
      { // skip
  ...
  return new Promise(resolve =>
  {
    ...
    for (let day = 1; day < colCounter; day++)
    {
      for (let bell = 0; bell < 7; bell++)
      {
        ...
        teacherEvents.push(
          parseTableCell(tableCells[index], url, day, time, extractUnitTeacherData)
        );
      }
    }
    ...
    try
    {
      ...
      action = myNsu.getFullName(shortName, 0, true);
    }
    catch (err)
    {
...
function handleGroupTable($, url)
{
  ...
  for (let day = 1; day < colCounter; day++)
  {
    for (let bell = 0; bell < 7; bell++)
    {
      ...
      groupEvents.push(
        parseTableCell(tableCells[index], url, day, time, extractUnitGroupData)
      );
    }
  }
...
function parseTableCell(cell, url, day, time, cb)
{
