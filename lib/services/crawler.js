...
function handleTable($, url)
{
  ...
  for ( let day = 1; day < colCounter; day++ )
  {
    for ( let bell = 0; bell < 7; bell++ )
    {
      ...
      groupEvents.push(
        ...
        teachersSet, placesSet)
      );
    }
  }

  // what should be recalculated
  for (let key of teachersSet.entries())
  {
    teachersForRecalculation.add(key);
  }
  for (let key of placesSet.entries())
  {
    placesForRecalculation.add(key);
  }

  if (temp && temp.teachers)
  {
    for (let key of temp.teachers.entries())
    {
      teachersForRecalculation.add(key);
    }
  }
  if (temp && temp.places)
  {
    for (let key of temp.places.entries())
    {
      placesForRecalculation.add(key);
    }
  }
...
function parseTableCell(cell, url, day, bell, teachersSet, placesSet)
{
