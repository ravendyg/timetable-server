...
const allQuery = 'SELECT * from sync WHERE status=1;';
const afterQuery = 'SELECT * from sync WHERE UNIX_TIMESTAMP(modified)>?;';
const allWithTimeQuery = 'SELECT * from sync WHERE status=1 AND time=?;';
const afterWithTimeQuery = 'SELECT * from sync WHERE UNIX_TIMESTAMP(modified)>? AND time=?;';
		...
		(resolve, reject) =>
		{
			connection.query(
				...
				(err, res) =>
				{
					if (err)
					{
						console.erro(err, getAfterThisTime, tmst, time);
						res = [];
					}
...
function check ( item )
{
	connection.query(
		'SELECT day, time, place, group_name, sync.name AS name, position, person AS personId, status, ' +
				...
				'temp.name as person, temp.full_name as fullName ' +
		'FROM sync ' +
			'INNER JOIN ( ' +
									'SELECT * FROM teachers UNION ALL SELECT 0 as id, \'\' as name, \'\' as full_name ' +
								') as temp ' +
				'ON sync.person=temp.id ' +
